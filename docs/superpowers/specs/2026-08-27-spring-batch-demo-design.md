...
`OrderRangePartitioner.partition(int gridSize)` queries `MIN(id)`/`MAX(id)` for `batch_type = 'PARTITION' AND status = 'PENDING'` orders and splits that range into `gridSize` contiguous id-range partitions, injecting `minId`/`maxId` into each partition's `ExecutionContext` (an empty order set falls back to one no-op partition — `minId=0, maxId=-1` — rather than crashing on a null `MIN`/`MAX`). The worker step's reader is `@StepScope` with `@Value("#{stepExecutionContext['minId']}")`/`maxId` late-binding, querying `... WHERE batch_type = 'PARTITION' AND status = 'PENDING' AND id BETWEEN ? AND ?` via `JdbcCursorItemReaderBuilder.preparedStatementSetter(...)`. `PartitionJobConfig` wires the master step via `StepBuilder(...).partitioner("partitionWorkerStep", partitioner).step(workerStep).gridSize(4).taskExecutor(new SimpleAsyncTaskExecutor())`, reusing `InvoiceProcessor`/`InvoiceItemWriter` from `chunk/`. Grid size is fixed at `4` in the step definition rather than a REST parameter — `TaskExecutorPartitionHandler`'s grid size is set at step-bean-definition time, not something a vanilla `Partitioner`/`PartitionHandler` re-reads per request without a custom `PartitionHandler` implementation, which is more machinery than this pattern needs to demonstrate. Requires the H2 datasource to support genuine concurrent connections (see Spring Boot configuration below) since partitions run their chunk steps in parallel.
...
| `POST /demo/batch/partition` | Launches `partitionedInvoiceJob` (unique timestamp parameter each call; grid size fixed at 4 partitions); returns `JobRunResult`. |
...
- **Job-config tests** (`chunk`, `tasklet`, `faulttolerant`, `restart`, `partition`) use `@SpringBootTest(classes = { ...only that job's own beans... })` + `@SpringBatchTest`, seeding orders directly via `JdbcTemplate` in `@BeforeEach`, launching the job under test via `JobLauncherTestUtils` with explicit unique `JobParameters` (or a fixed `runId` for the restart test, launched twice within the same test to assert `FAILED` then `COMPLETED`), and asserting on the returned `JobExecution`'s status/counts plus the resulting `orders`/`invoices` rows. Narrowing `@SpringBootTest`'s `classes` attribute to each test's own `@Configuration`/`@Component` set (rather than loading the full application) is required, not just tidy: with all five `Job` beans in one full application context, `@SpringBatchTest`'s auto-wired `JobLauncherTestUtils` cannot unambiguously resolve which `Job` to bind. Spring Boot's auto-configuration (`DataSource`, `JobRepository`, `PlatformTransactionManager`, batch schema init) still applies normally under `@SpringBootTest(classes = ...)` — only component scanning is narrowed. This is a real, necessary deviation from this repo's usual pure-Mockito unit-test convention — Spring Batch job wiring (readers/processors/writers/listeners composed into a `Step`/`Job`) is declarative Spring configuration that isn't meaningfully testable by mocking `JobRepository`/`JobLauncher`; `@SpringBatchTest` is the framework's own documented testing approach for exactly this. `distributed-transactions/saga` already has a precedent for integration-style testing in this repo (`SagaIntegrationTest`, a `@WebMvcTest` + `@Import` of real collaborator beans), though Spring Batch's need for a real `DataSource`/`JobRepository` pushes this further to full `@SpringBootTest`.
