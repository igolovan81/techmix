...
## Post-implementation notes (from Task 13 live verification)

`AggregateTestFixture`/`@WebMvcTest`/Mockito-based unit tests (Tasks 5–9) never serialize a message or talk to a real Axon Server, so they couldn't catch three issues that only appeared once the app ran against a live Axon Server in Task 13. All three are now fixed in the codebase; documented here so the plan stays an accurate record:

1. **Records aren't serializable by Axon's default XStream serializer.** Fixed by setting `axon.serializer.general/events/messages: jackson` in `application.yml` (Jackson supports records natively since 2.12).
2. **Jackson can't read `OrderAggregate`'s private fields for snapshotting** without getters. Fixed with `@JsonAutoDetect(fieldVisibility = JsonAutoDetect.Visibility.ANY)` on the aggregate class. Without it, the snapshot threshold (5th event) throws `IncompatibleAggregateException`.
3. **`List<OrderSummary>` returned directly from `@QueryHandler` loses its element type over the wire** (generic erasure through `MultipleInstancesResponseType`), causing `MultipleInstancesResponseType.convert()` to reject a deserialized list of raw maps. Fixed by introducing `OrderSummaries(List<OrderSummary> orders)` as a single-instance query response wrapper instead — `OrderProjection.handle(FindAllOrdersQuery)` now returns `OrderSummaries`, and `DemoController.getAllOrders()` calls `.orders()` to keep the REST API shape (a plain JSON array) unchanged.

Also: business-rule violations in `OrderAggregate` (`ConfirmOrderCommand`/`CancelOrderCommand` on an already-confirmed order) now throw `CommandExecutionException(message, null, "BUSINESS_RULE_VIOLATION")` instead of a plain `IllegalStateException`. Axon's `CommandSerializer` only carries the *message text* of arbitrary exceptions across the Axon Server boundary by default (logged explicitly as "recommended to wrap the exception in a CommandExecutionException with provided details") — a `getCause() instanceof IllegalStateException` check on the receiving side never matched, so `DemoExceptionHandler` always fell through to 500. It now checks `exception.getDetails().isPresent()` instead.

Port note: the design's original `8086` choice (mentioned as "next free slot after MongoDB's 8084") was wrong — `8085` is already host-mapped to `message-brokers/pulsar`'s admin HTTP endpoint. `8086` is correct and is what's actually configured; this was caught and fixed during Task 11, before Task 13.

All 29 unit tests pass; the full order lifecycle (create → 6 lines → snapshot at line 5 → confirm → query → replay → both 409 rejections) was verified end-to-end against a live Axon Server.
