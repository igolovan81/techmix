...
public class RoutingConsumerAll implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) {
        ServiceBusProcessorClient processorClient = clientBuilder
                ...
                .subscriptionName(EntityNames.ROUTING_SUB_ALL)
                .processMessage(ctx -> {
                    String level = String.valueOf(
                            ctx.getMessage().getApplicationProperties().get(EntityNames.ROUTING_KEY));
                    log.info("[routing][sub-all] level={} received={}", level,
