...
@Slf4j
public class PendingConsumer implements StreamListener<String, MapRecord<String, String, String>> {
	...
	public void registerWith(StreamMessageListenerContainer<String, MapRecord<String, String, String>> container) {
		container.receive(Consumer.from(GROUP, CONSUMER),
				StreamOffset.create(StreamKeys.PENDING, ReadOffset.lastConsumed()), this);
	}
	...
	public void onMessage(MapRecord<String, String, String> record) {
		try {
			process(record);
			redisTemplate.opsForStream().acknowledge(StreamKeys.PENDING, GROUP, record.getId());
	...
	public void reclaimPending() {
		try {
			PendingMessages messages = redisTemplate.opsForStream().pending(StreamKeys.PENDING,
					Consumer.from(GROUP, CONSUMER), Range.unbounded(), 10L);
			...
			for (PendingMessage pm : messages) {
				if (pm.getElapsedTimeSinceLastDelivery().toMillis() >= MIN_IDLE) {
					List<MapRecord<String, Object, Object>> claimed = redisTemplate.opsForStream()
							.claim(StreamKeys.PENDING, GROUP, CONSUMER, Duration.ofMillis(MIN_IDLE), pm.getId());
					...
					for (var rec : claimed) {
						try {
							...
							redisTemplate.opsForStream().acknowledge(StreamKeys.PENDING, GROUP, rec.getId());
