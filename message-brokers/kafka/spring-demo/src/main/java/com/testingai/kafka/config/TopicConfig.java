...
public class TopicConfig {
	...
	public org.apache.kafka.clients.admin.NewTopic compactedTopic() {
		return TopicBuilder.name(COMPACTED_TOPIC).partitions(1).replicas(replicationFactor)
				.config("cleanup.policy", "compact").config("segment.ms", "5000")
				.config("min.cleanable.dirty.ratio", "0.01").build();
	}
	...
	@Bean
	public DefaultKafkaProducerFactory<String, String> transactionalProducerFactory(KafkaProperties kafkaProperties) {
		var factory = new DefaultKafkaProducerFactory<String, String>(kafkaProperties.buildProducerProperties(null));
	...
	@Bean
	public ConsumerFactory<String, String> readCommittedConsumerFactory(KafkaProperties kafkaProperties) {
