	...
	public void receive(String message, Channel channel, @Header(AmqpHeaders.DELIVERY_TAG) long deliveryTag,
		...
		try {
			...
			if (retryCount >= PubSubConfig.MAX_RETRIES) {
				log.error("[PubSubConsumerB] Max retries ({}) exceeded, discarding: {}", PubSubConfig.MAX_RETRIES,
						message);
				channel.basicAck(deliveryTag, false);
			} else {
				log.warn("[PubSubConsumerB] Failed (retry {}/{}), sending to retry queue: {}", retryCount + 1,
